...
Кадры всех 8 сцен ≈ **22 МБ** (ширина 1000px). Вся лента **грузится целиком за
прелоадером** (прогресс-бар с %), и только потом открывается сайт — так скролл
никогда не обгоняет загрузку и не «зависает» на кадре. На быстром канале старт —
несколько секунд. Хочешь быстрее старт ценой резкости — пересобери кадры уже
(`WIDTH=880 QV=7`). Исходные видео были ~100 МБ.

Производительность скраба: плашки без `backdrop-filter` (он пересчитывал blur
каждый кадр и был главным тормозом), кадры декодируются на несколько штук вперёд.
